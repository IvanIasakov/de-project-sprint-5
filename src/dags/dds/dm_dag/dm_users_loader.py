...
class UsersOriginRepository:
    ...
    def list_users(self, user_threshold: int, limit: int) -> List[UserObj]:
        with self._db.client().cursor(row_factory=class_row(UserObjStg)) as cur:
            cur.execute(
                """
            ...
            for record in objsin:
                 ...
                 objs.append(UserObj(**{'id':record.id,'xid':el['_id'],'login':el['login'],'name':el['name'],'update_ts':el['update_ts']}))
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                    ...
                    SET user_id=EXCLUDED.user_id,user_name = EXCLUDED.user_name, user_login = EXCLUDED.user_login;
                """,
                {
                    "user_id": user.xid,
                    "user_name": user.name,
