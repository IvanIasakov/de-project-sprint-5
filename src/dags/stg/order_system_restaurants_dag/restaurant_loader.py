...
class RestaurantLoader:
    ...
    def run_copy(self, tNam: str) -> int:
        ...
        with self.pg_dest.connection() as conn:
            ...
            wf_setting = self.settings_repository.get_setting(conn, self.WF_KEY.replace("restaurants",tNam))
            if not wf_setting:
                wf_setting = EtlSetting(
                    id=0,
                    workflow_key=self.WF_KEY.replace("restaurants",tNam),
                    workflow_settings={
            ...
            load_queue = self.collection_loader.get_restaurants(last_loaded_ts, self._SESSION_LIMIT, tNam)
            ...
            i = 0
            for d in load_queue:
                self.pg_saver.save_object(conn, str(d["_id"]), d["update_ts"], d, tNam)
